...
redirect_from:
  - /publications
---
<script> function button(id) { var x = document.getElementById(id); var ids = ["abs1"]; for(var i = 0; i < ids.length; i++) { var item = ids[i]; if (item != id) { document.getElementById(item).style.display = "none"; } else { if (x.style.display === "none") { x.style.display = "block" } else { x.style.display = "none"; } } } } </script>

<p style="margin-bottom:0">Test Paper (with Test Authors)</p>
<div class="buttonbar">[
    <button class="button" onclick="button(&quot;abs1&quot;)">abstract</button>
  ]</div>
  <div class="popup" id="abs1" style="display:none">Test abstract</div>
<br>
